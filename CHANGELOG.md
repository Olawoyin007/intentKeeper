...
- Added 6 YouTube hype examples to `eval/test_set.yaml` - shock-word/ALL-CAPS titles, vague-payoff "REAL reason"/"truth they don't want you to see" clickbait, manufactured FOMO ("watch before it gets taken down"), and superlative overload - plus one genuine boundary case (a real benchmark deep-dive that only sounds like clickbait but cites specifics). YouTube titles are the richest clickbait source and were underrepresented in the `hype` set; all six classify correctly (hype subset 19/19 on `llama3.1:8b`) (closes #101)

### Docs
- `THREAT_MODEL.md`: added an "Engineering controls" table mapping each protection (local Ollama only, SSRF allowlist, prompt-injection boundary, correction-injection guard, bounded request bodies, DOM-XSS escaping, extension-talks-only-to-its-own-server, fail-open) to the file that enforces it, so reviewers can verify a change has not weakened a control. Mirrors the same addition in empathySync. Also corrected the `chrome-extension://*` CORS known-gap: it was described as allowing *any* installed extension (over-permissive), but Starlette matches `allow_origins` by exact string, so the literal `chrome-extension://*` matches no real extension origin (verified empirically) and CORS actually fails closed for extension origins - the extension reaches the server via MV3 `host_permissions`, not CORS. Reframed as misleading dead config and cross-linked the cleanup (#113). From a periodic security re-audit (2026-06-30) that found no new exploitable issues
...
### Infrastructure
- CI: the `validate-description` check now skips Dependabot PRs (`if: github.actor != 'dependabot[bot]'`). Dependabot's auto-generated bodies never include our `## Summary`/`## Testing` sections, so the check failed on every dependency bump and required a manual body edit before merge. Dependabot PRs are now mergeable without that friction
- CI now runs the extension Jest suite (`npm ci && npm test` in the `extension-lint` job, gated by the same docs-only skip). Previously only `pytest` and manifest validation ran in CI, so JS test failures were invisible. Fixing that surfaced a jsdom-version bug: the old-Reddit `extractText` tests mocked `window.location` at runtime, which throws under jsdom 26 (non-configurable `location`) on a clean `npm ci`. Moved those into `reddit.oldreddit.test.js` with a file-level `old.reddit.com` URL instead. Full suite now 77/77 under a clean install
